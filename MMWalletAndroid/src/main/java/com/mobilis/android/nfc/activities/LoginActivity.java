...
import com.mobilis.android.nfc.model.Tap;
import com.mobilis.android.nfc.model.TransferCode;
import com.mobilis.android.nfc.slidemenu.utils.LoginResponseConstants;
import com.mobilis.android.nfc.tasks.DeviceSyncTask;
import com.mobilis.android.nfc.tasks.LoadingCodesTask;
import com.mobilis.android.nfc.tasks.LoadingCurrencyOnly;
import com.mobilis.android.nfc.tasks.TopUpAirtimeCodeTask;
import com.mobilis.android.nfc.tasks.TopUpCreditCodeTask;
import com.mobilis.android.nfc.tasks.UtilityCodeTask;
...
import com.mobilis.android.nfc.util.NFCTagUtil;
...
//        }, 2000);
        ...
        if(getResources().getBoolean(R.bool.LOAD_TRANSFERCODES_FROM_SERVER)) {
            if(!LoadingCurrencyOnly.processd) {
                LoadingCurrencyOnly lco= new LoadingCurrencyOnly();
                lco.execute(this.getApplicationContext());
            }
        }else{
            if(!LoadingCodesTask.processd) {
                LoadingCodesTask lct = new LoadingCodesTask();
                lct.execute(this.getApplicationContext());
            }
        }
    ...
    private void setServerIp(){
        String port = mContext.getResources().getString(R.string.SERVER_PORT);
        String SERVER_IP = getString(R.string.SERVER_IP);
        login.saveIPAddress(SERVER_IP);
        login.savePort(port);
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if(hasNFCFeature()) {
            try {
                ...
                String nfcId = NFCTagUtil.findTagId(tag).toUpperCase(Locale.US);//AndroidToAndroidNFCActivityLowerVersions.onTagDiscovered(intentTag, this).toUpperCase(Locale.US);
                Log.d(TAG, "Posting to server with message type of TAP " + nfcId);
                Tap tap = new Tap(this);
                tap.setNFCId(nfcId);
                tap.getConnTaskManager().startBackgroundTask();
            }catch(Exception e){
    ...
    protected void onResume() {
        ...
        broadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                }   else if(action.equalsIgnoreCase(INTENT.TAP.toString())){

                    String serverResponse = intent.getStringExtra(INTENT.EXTRA_SERVER_RESPONSE.toString());
                    String status = intent.getStringExtra(getString(R.string.REQ_MESSAGE_TYPE_TAP_STATUS));
                    String serverMessage= intent.getStringExtra(getString(R.string.REQ_MESSAGE_TYPE_TAP_MESSAGE));
                    Log.d(TAG,"TAP response status"+status);
                    Log.d(TAG,"TAP response serverMessage "+serverMessage);
                    Log.d(TAG,"TAP response serverResponse"+serverResponse);
                    showTapResponse(status,serverMessage,serverResponse);
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(broadcastReceiver, new IntentFilter(INTENT.TAP.toString()));
        if(hasNFCFeature()) {
    ...
    private void syncWithServer(){
        try {
            ...
            if (allDataSet != null && allDataSet.size() > 0) {
                Dataset oneList = null;
                for (DatasetMetadata oneMeta : allDataSet) {
                    String dataName = oneMeta.getDatasetName();
                    Log.d("TAG", "oneMeta dataName : " + dataName);

                    oneList = client.openOrCreateDataset(dataName);
                    Log.d("TAG", "oneMeta oneList : " + oneList);
                }
                oneList = client.openOrCreateDataset(mContext.getString(R.string.GLOBAL_DATASET_NAME));
                oneList.synchronize(new Dataset.SyncCallback() {
                    @Override
                    public void onSuccess(Dataset dataset, List<Record> updatedRecords) {
                        Log.d("TAG", "synchronize onSuccess  : ");
                        for (Record oneRecord : updatedRecords) {
                            String key = oneRecord.getKey();
                            String value = oneRecord.getValue();
                            Log.d("TAG", "updatedRecords oneRecord : " + key + ".." + value);
                        }
                        for (Record oneRecord : dataset.getAllRecords()) {
                            String key = oneRecord.getKey();
                            String value = oneRecord.getValue();
                            Log.d("TAG", "dataset oneRecord : " + key + ".." + value);
                        }
                        if (dataset != null && dataset.getAllRecords().size() > 0) {
                            SyncDeviceId(dataset);
                        }
                    ...
                    public boolean onConflict(Dataset dataset, List<SyncConflict> conflicts) {
                        Log.d("TAG", "synchronize onConflict  : ");
                        return false;
                    }

                    @Override
                    public boolean onDatasetDeleted(Dataset dataset, String datasetName) {
                        Log.d("TAG", "synchronize onDatasetDeleted  : ");
                        return false;
                    }

                    @Override
                    public boolean onDatasetsMerged(Dataset dataset, List<String> datasetNames) {
                        Log.d("TAG", "synchronize onDatasetsMerged  : ");
                        return false;
                    }

                    @Override
                    public void onFailure(DataStorageException dse) {
                        Log.d("TAG", "synchronize onFailure  : ");
                    }
                });
            ...
            Log.e("TAG","error syncing device Id"+e.toString());
        }
    }
    private void SyncDeviceId(Dataset dataset) {
        try {
            boolean deviceHasBeenSynced = mainSecurePreferences.getBoolean("DeviceSynced", false);
            String merchantId = login.getMerchantId();
            String androidId = login.getAndroidId(mContext);
            String imei = "";
            ...
            if (merchantId != null) {
                if (!deviceHasBeenSynced) {
                    String URL = dataset.get(mContext.getString(R.string.GLOBAL_RECORD_URL));
                    String apiKey = dataset.get(mContext.getString(R.string.GLOBAL_RECORD_APIKEY));
                    Log.d("TAG", "SyncDeviceId URL : " + URL);
                    Log.d("TAG", "SyncDeviceId apiKey : " + apiKey);
                    DeviceSyncTask dst = new DeviceSyncTask();
                    dst.execute(imei, merchantId, androidId, mContext, URL, apiKey);
                }
            }
        }catch(Exception e){
            Log.d("TAG", "SyncDeviceId error : " + e.toString());
        }
    }
...
/*    private void playSoundPool(){
    ...
    private void showTapResponse(String status, String serverMessage, String serverFullResponse){
        final Dialog aboutDialog = new Dialog(mContext);
        aboutDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        aboutDialog.setContentView(R.layout.about_dialog);
        TextView androidDeviceId = ((TextView)aboutDialog.findViewById(R.id.Dialog_About_TextView_Content));
        androidDeviceId.setTextSize(14);
        String details ="Status : "+status+"\n\nMessage :\n"+serverMessage+"\n\n Full Response :\n"+serverFullResponse;
        androidDeviceId.setText(details);
        aboutDialog.show();
        TimerTask timerTask = new TimerTask() {
            @Override
            public void run() {
    ...
    private int currVolume=80;
   /* private void initMediaPlayer(){
        mediaPlayer = MediaPlayer.create(mActivity, R.raw.chime);
    }
    AudioManager.OnAudioFocusChangeListener audioFocusChangeListener = new AudioManager.OnAudioFocusChangeListener() {
        @Override
        public void onAudioFocusChange(int focusChange) {
            Log.d(TAG,"onAudioFocusChange AudioManager.AUDIOFOCUS_GAIN "+AudioManager.AUDIOFOCUS_GAIN);
            Log.d(TAG,"onAudioFocusChange AudioManager.AUDIOFOCUS_LOSS"+ AudioManager.AUDIOFOCUS_LOSS);
            Log.d(TAG,"onAudioFocusChange AudioManager.AUDIOFOCUS_LOSS_TRANSIENT"+AudioManager.AUDIOFOCUS_LOSS_TRANSIENT);
            Log.d(TAG,"onAudioFocusChange AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK"+AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK);
            Log.d(TAG,"onAudioFocusChange current "+focusChange);
            switch (focusChange) {
    ...
    public void onClick(View view) {
        ...
        if(view.getId() == R.id.Activity_Login_Button_Register){
            startActivity( new Intent(LoginActivity.this, SocialIdRegistration.class));
            //startActivity( new Intent(LoginActivity.this, MultiRegistrationAcitivity.class));
...
/*            for(int i=0;i<textAllPins.getText().length();i++)
            {
                ImageView ii= new ImageView(this);
                ii.setBackgroundResource(R.drawable.lock);
                ii.setwi
                allPinsLayout.addView(ii);
            }*/
            showLogin();
        }else{
           // allPinsLayout.removeAllViewsInLayout();//.removeAllViews();
            hideLogin();
        }
    }
    ...
    private void setupComponents() {
        ...
        loginTitleText = (TextView) findViewById(R.id.Login_title_text);
      ...
      //  allPinsLayout = (LinearLayout) findViewById(R.id.activity_login_all_pins_layout);
        loginTitleText.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                ...
                aboutDialog.setContentView(R.layout.about_dialog);
                TextView androidDeviceId = ((TextView)aboutDialog.findViewById(R.id.Dialog_About_TextView_Content));
                androidDeviceId.setTextIsSelectable(true);
                ...
                String SERVER_IP =mContext.getString(R.string.SERVER_IP);
                String SERVER_PORT =mContext.getString(R.string.SERVER_PORT);
                ...
                String details =mContext.getString(R.string.app_name);
                details+= "\n "+ SERVER_IP;
                details+= "\n "+ SERVER_PORT;
                details+= "\n "+ mContext.getApplicationContext().getPackageName();
                details+= "\n "+ Settings.System.getString(mContext.getContentResolver(), Settings.Secure.ANDROID_ID).toUpperCase(Locale.US);
                details+= "\n "+  Boolean.parseBoolean(mContext.getString(R.string.ONBOARD_NFC));
                details+= "\n Version : "+  mContext.getString(R.string.APP_VERSION_INTERNAL);
                details+= " , "+  mContext.getResources().getInteger(R.integer.APP_VERSION_CODE);
                androidDeviceId.setText(details);
                aboutDialog.show();
                return false;
            }
   ...
   // private LinearLayout allPinsLayout;
    ...
    protected void onPause() {
        super.onPause();
        LocalBroadcastManager.getInstance(this).unregisterReceiver(broadcastReceiver);
        Log.d(this.getClass().getSimpleName(),"LoginActivity onPause");
