...
public class TagRegistrationFragment extends NFCFragment implements View.OnClickListener, DialogInterface.OnDismissListener, DialogInterface.OnShowListener {
    ...
    RelativeLayout tagOwnerLayout;
    EditText tagOwnerName;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        tagOwnerLayout = (RelativeLayout) rootView.findViewById(R.id.Fragment_TagRegistration_TagOwnerLayout);
        tagOwnerName = (EditText) rootView.findViewById(R.id.Fragment_TagRegistration_TagOwnerNameET);
        ...
        pinET.addTextChangedListener(new TextWatcher() {
            ...
            @Override
            public void afterTextChanged(Editable s) {
                //int pinLength = s.length();//getResources().getInteger(R.integer.PIN_LENGTH);
                resultTV.setVisibility(View.GONE);
/*                if(pinLength>0){
                    pinTV.setText("");
                }else{
                    pinTV.setText(getResources().getString(R.string.pin));
                }*/
    ...
    private boolean displayTagOwnerNameField = false;

    private void checkingConfigurationSettings() {
        Log.d(TAG, "testing checkingConfigurationSettings getGENERALFLAGS " + ApplicationActivity.configurationVariablesModel.getGENERALFLAGS());
        Log.d(TAG, "testing checkingConfigurationSettings BitsMapper.GENERALFLAGS_TAGOWNERNAME " + BitsMapper.GENERALFLAGS_TAGOWNERNAME);
        Log.d(TAG, "testing checkingConfigurationSettings 16 " + BitsMapper.parseBitSet(ApplicationActivity.configurationVariablesModel.getGENERALFLAGS(), BitsMapper.GENERALFLAGS_TAGOWNERNAME, 16));
        displayTagOwnerNameField = BitsMapper.parseBitSet(ApplicationActivity.configurationVariablesModel.getGENERALFLAGS(), BitsMapper.GENERALFLAGS_TAGOWNERNAME, 16);
    }
    ...
    public void onResume() {
        ...
        checkingConfigurationSettings();
        if (displayTagOwnerNameField) {
            tagOwnerLayout.setVisibility(View.VISIBLE);
        } else {
            tagOwnerLayout.setVisibility(View.GONE);
        }
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent.getAction().equalsIgnoreCase(INTENT.TAG_REGISTRATION.toString())) {
                    ...
                    if (NFCDialog != null && NFCDialog.isShowing()) {
                        NFCDialog.dismiss();
                        if (intent != null) {
                            Log.d(TAG, " nfc intent " + intent);
                            String tagId = intent.getStringExtra(INTENT.EXTRA_NFC_ID.toString());
                            Log.d(TAG, " nfc tagId " + tagId);
                            if (tagId != null && !tagId.isEmpty()) {
                                ...
                                model.setNFCId(tagId);
                                if (displayTagOwnerNameField) {
                                    model.setTagOwnerName(tagOwnerName.getText().toString());
                                }
                                model.getConnTaskManager().startBackgroundTask();
                            } else {
                                Log.d(TAG, " nfc tagId " + tagId);
                                hideProgressBar("Error reading tag, please try again.");
                            }
                        } else {
                            Log.d(TAG, " nfc intent is null ");
                            hideProgressBar("Error reading tag, please try again.");
                        }
                    }
                }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.Fragment_TagRegistration_Button_Submit:
                ...
                if (displayTagOwnerNameField) {
                    if (tagOwnerName.getText().toString().isEmpty()) {
                        tagOwnerName.setError("Mandatory field");
                        return;
                    }
                }
