...
//                getLastTransactions();
            ...
            getLastTransactions();
           /* if(model.getActivity().getResources().getBoolean(R.bool.LOAD_TRANSFERCODES_FROM_SERVER)) {
    ...
    private boolean checkAccountTabExists(){
        /** no need to check for account tab, always exists 02032015*/
        boolean temp = false;
        String menuC = getMenuContents(model.getServerResponse(), model.getResString(R.string.MENU_C));
  ...
  /*    Log.d("RUA", "model.getServerResponse(): "+model.getServerResponse());
        ...
        if(menuD != null) {
            temp = isShowBalanceAvailable(menuD);
            Log.d("RUA", "isShowBalanceAvailable: " + temp);
        }*/
        temp = isShowBalanceAvailable(menuC, menuD);
        return temp;
    }
    ...
    private class BuildLoggingRequirementsTask extends AsyncTask<Void, Void, Void>{
        ...
        private void setupProfileMenus() {
            ...
            Log.d(LoginResponse.class.getSimpleName(), "setting up setupProfileMenus " );
