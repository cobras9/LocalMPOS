                        ...
                        if(model != null && model.isC2MPTransaction())
                        {
                            ...
                            if (getResources().getBoolean(R.bool.DEMO_WALLET_WAIT_FOR_PIN)) {
                                if(ApplicationActivity.isDevice) {
                                    ...
                                    //pst.executeOnExecutor(AsyncTask.SERIAL_EXECUTOR);
...
/*        if(buttonOrder == 1){
        ...
        setUpQLDetailsDialog(qpDialog, buttonOrder - 1);
        qpDialog.show();
    }
...
//                    qpDialog.dismiss();
    ...
    private void setUpQLDetailsDialog(final Dialog qpDialog, final int index){
        ...
        closeOKBTN = (Button) qpDialog.findViewById(R.id.Dialog_QuickLink_C2MP_CLOSE_OK);
        qpDialog.setOnDismissListener(this);
        closeOKBTN.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (qpDialog.isShowing()) {
                    qpDialog.dismiss();
                }
            }
        });
        closeOKBTN.setVisibility(View.GONE);
    ...
    private void showNFCOnlyDialog(int index, TextView amountTV, TextView phoneNumberTV) {
        ...
        resultTV.setBackgroundColor(getResources().getColor(R.color.QUICKLINK_BACKGROUND_COLOR));
        resultTV.setTextColor(getResources().getColor(R.color.QUICKLINK_TEXT_COLOR));
        resultTV.setGravity(Gravity.CENTER);

        phoneNumberET.setText(Constants.quickLinks.get(index).getAmount());
        phoneNumberET.setGravity(Gravity.CENTER);
        phoneNumberET.setTextSize(32);
        phoneNumberET.setTextColor(getResources().getColor(R.color.QUICKLINK_TEXT_COLOR));
        phoneNumberET.setTypeface(null, Typeface.BOLD);
        phoneNumberET.setEnabled(false);
        phoneNumberET.setBackgroundColor(getResources().getColor(R.color.QUICKLINK_BACKGROUND_COLOR));
        enableNFCScan();
    }
...
/*        pinEditText.addTextChangedListener(new TextWatcher() {
    ...
    private void hideProgressBar(String response){
        ...
        if(resultTV != null)
        {
            ...
            resultTV.setText(response);
            resultTV.setVisibility(View.VISIBLE);
            closeOKBTN.setVisibility(View.VISIBLE);
        }
    }
