...
PREVTAG=${PREVTAG:4:3}
TAG=$((PREVTAG+1))
APP_VERSION_INTERNAL='1.2.'$TAG
FINAL_PROFILE=$4
nfc_enabled=$5
optional_server=$6
optional_port=$7
need_mobmonpin_instead_of_ussd=$8
load_transfer_coder_from_server=$9
...
#i=0;
#for file in $(grep --include=\*.xml -l -R 'ONBOARD_NFC' ./MMWalletAndroid/res/)
#do
...
#i=0;
#for file in $(grep --include=\*.xml -l -R 'PAY_PIN_REQUIRED' ./MMWalletAndroid/res/)
#do
#  sed "s/.*name=.*\"PAY_PIN_REQUIRED\".*/<bool translatable=\"false\" name=\"PAY_PIN_REQUIRED\">$need_mobmonpin_instead_of_ussd<\/bool>/g" $file > tempfile.tmp && mv tempfile.tmp $file
#    let i++;
#  echo "Modified USSD pin entry value: " $file
#done
